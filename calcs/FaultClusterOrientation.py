...
class FaultsOrientations:
    ...
    def read_trend_from_shapefile(self, shapefile_path, trend_field="trend"):
        ...
        try:
            ...
            if trend_field not in gdf.columns:
                ...
                raise ValueError(
                    f"Field '{trend_field}' not found. Available fields: {available_fields}"
                )
    ...
    def find_optimal_clusters(self, azimuths, max_clusters=40):
    ...
    def analyze_shapefile_trends(
        ...
        for i in range(n_clusters):
            ...
            print(
                f"  Cluster {i}: {centers[i]:.1f}° ({stats['count']} points, "
        ...
        gdf_with_clusters.loc[valid_indices, "cluster_center"] = [
    ...
    def plot_circular_clusters(
        self, azimuths, labels, cluster_centers, title="Circular K-Means Clustering"
    ):
        ...
        # Linear histogram
        ax2.hist(
            [azimuths[labels == i] for i in range(len(np.unique(labels)))],
            bins=20,
            alpha=0.7,
            label=[f"Cluster {i}" for i in range(len(np.unique(labels)))],
            color=colors,
        )
    ...
    def plot_spatial_clusters(self, gdf_with_clusters, trend_field="trend"):
        ...
        gdf_with_clusters.plot(
            column=trend_field, ax=ax2, markersize=50, cmap="viridis", legend=True
        )
    ...
    def example_basic_usage(self):
        ...
        try:
            ...
            for stats in results["cluster_stats"]:
                print(
                    f"Cluster {stats['cluster_id']}: {stats['count']} points "
            ...
            print(
                "Please update the shapefile_path variable with your actual file path."
            )
            return None
        except Exception as e:
            print(f"Error: {e}")
    ...
    def create_custom_plots(self, gdf, trend_data, labels, centers):
        ...
        for i, color in enumerate(colors):
            ...
            hist, bin_edges = np.histogram(
                cluster_data_180, bins=n_bins, range=(0, 180)
            )
        ...
        bp = ax3.boxplot(
            cluster_data,
            labels=[f"C{i}" for i in range(len(cluster_data))],
            patch_artist=True,
        )
    ...
    def example_custom_analysis(self, results):
        ...
        for i in range(results["n_clusters"]):
            ...
            print(
                f"  Count: {len(cluster_trends)} ({len(cluster_trends)/len(trend_data)*100:.1f}%)"
            )
            print(
                f"  Range: {cluster_trends.min():.1f}° to {cluster_trends.max():.1f}°"
            )
    ...
    def add_endpoint_azimuth(
        self, shapefile_path, output_path=None, azimuth_field="end_azimuth"
    ):
        """
    ...
    def demonstrate_nondirectional_symmetry(self):
        ...
        hist_wrong, bins_wrong = np.histogram(
            angles_wrong, bins=36, range=(0, 4 * np.pi)
        )
        ...
        for height, angle in zip(hist_correct, bin_centers_rad):
            if height > 0:
                ...
                ax2.bar(
                    angle + np.pi, height, width=width_correct, alpha=0.7, color="green"
                )
    ...
    def example_manual_clusters(
        self, shapefile_path, layer_name, azimuth_field="endpt_az"
    ):
        ...
        try:
            ...
            if showPlots:
                plt.figure(figsize=(10, 6))
                plt.plot(
                    cluster_numbers, silhouette_scores, "bo-", linewidth=2, markersize=8
                )
                plt.xlabel("Number of Clusters")
                plt.ylabel("Silhouette Score")
