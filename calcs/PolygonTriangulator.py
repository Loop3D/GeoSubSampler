...
class PolygonTriangulator:
    def __init__(self, shapefile_path=None, gdf=None, id_column=None,min_area_threshold=1.0 , distance_threshold=1.0,
                            strat1="", strat2="", strat3="", strat4="", lithoname=""):
        """
                ...
                if( row.geometry.area < self.min_area_threshold):
                    ...
                    for triangle in triangles:
                        ...
                        for field_name, field_type in field_config.items():
                            triangle_record[field_name] = self._generate_random_value(field_type)
    ...
    def _triangulate_with_triangle_library(self, polygon):
        ...
        for hole in polygon.interiors:
            ...
            for i in range(len(hole_coords)):
                segments.append([hole_start_idx + i, hole_start_idx + ((i + 1) % len(hole_coords))])
    ...
    def visualize_triangulation(self, result_gdf, title="Triangulated Polygons", figsize=(15, 10)):
        """
        ...
        self.gdf.plot(ax=ax1, alpha=0.6, edgecolor='black', linewidth=1, 
                     column=self.id_column, cmap='Set3', legend=False)
        ...
        if not result_gdf.empty:
            ...
            if not triangles.empty:
                triangles.plot(ax=ax2, alpha=0.7, edgecolor='red', linewidth=0.5,
                              color='lightcoral', label='Triangles')
            
            if not originals.empty:
                originals.plot(ax=ax2, alpha=0.6, edgecolor='blue', linewidth=1,
                              color='lightblue', label='Original')
            
            ax2.legend()
            ax2.set_title(f"Combined Result ({len(triangles)} triangles + {len(originals)} originals)")
        else:
...
def create_sample_data():
    ...
    # Pentagon
    pentagon = Polygon([
        (250, 50), (280, 20), (320, 40), (310, 80), (260, 85)
    ])
...
if __name__ == "__main__":
    ...
    try:
        if Path(shapefile_path).exists():
            ...
            triangulator = PolygonTriangulator(shapefile_path=shapefile_path, id_column='CODE')
        else:
    ...
    print("Available polygon IDs:", list(triangulator.gdf[triangulator.id_column].unique()))
    ...
    try:
        ...
        if not result_gdf.empty:
            ...
            if not triangles.empty:
                print(triangles[['rec_id', 'tri_id', 'src_id', 'area'] + list(field_config.keys())].head(3))
            
            print("\nSample original polygon data:")
            if not originals.empty:
                print(originals[['rec_id', 'orig_id', 'area'] + list(field_config.keys())].head(3))
